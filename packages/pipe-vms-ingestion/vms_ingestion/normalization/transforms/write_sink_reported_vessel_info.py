...
def table_schema():
    return query.get_schema("assets/feeds/reported_vessel_info.schema.json")
...
def table_descriptor(destination, labels, schema=table_schema()):
    ...
    table.clustering_fields = ["source_tenant", "ssvid"]
...
class WriteSinkReportedVesselInfo(beam.PTransform):
    def __init__(
        self,
        destination,
        labels,
    ...
    def write_sink(self):
        print(
            f"Writing Sink to {self.table.project}.{self.table.dataset_id}.{self.table.table_id}"
        )
