...
def excel_to_bq(argv):
    ...
    options = build_pipeline_options_with_defaults(argv).view_as(
        IngestionExcelToBQOptions
    )
    ...
    if is_blocking_run(options, IngestionExcelToBQOptions):
        logging.info("Waiting for job to finish")
        result.wait_until_finish()

    logging.info("Pipeline launch result %s", result.state)

    if result.state in success_states(options, IngestionExcelToBQOptions):
